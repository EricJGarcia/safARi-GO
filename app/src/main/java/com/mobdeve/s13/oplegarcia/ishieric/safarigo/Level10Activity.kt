...
class Level10Activity : AppCompatActivity() {
    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Swordfish" -> "Hint: I have a long, pointed sword nose."
                "Jellyfish" -> "Hint: I float in the ocean with long, stinging tentacles."
                "Walrus" -> "Hint: I have large tusks and whiskers, and I live in cold waters.\n"
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            currentIndex = 0
            Toast.makeText(this, "Level Completed! CONGRATULATIONS!!!", Toast.LENGTH_SHORT).show()
            Handler(Looper.getMainLooper()).postDelayed({
                val intent = Intent(this, Level10Activity::class.java)

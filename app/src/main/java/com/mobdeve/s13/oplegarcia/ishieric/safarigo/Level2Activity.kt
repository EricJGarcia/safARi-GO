...
class Level2Activity : AppCompatActivity() {
    ...
    private var targetItem = "Cat" // Initial target for this level
    private var points = 0

    // List of animals for each level, cycling through them
    private val animals = listOf("Cat", "Camel", "Cow")
    private var currentIndex = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_g1level2) // Ensure this is the correct layout
    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Cat" -> "Hint: I purr when I’m happy and have retractable claws. C _ _"
                "Camel" -> "Hint: I have humps on my back to store fat for long desert journeys. C _ M _ L"
                "Cow" -> "Hint: I produce milk and say moo. C _ _"
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            currentIndex = 0
            dbHelper.setLevelCompleted("Level 2")
            Toast.makeText(this, "Level Completed! Proceed to Level 3 Yipee!!!", Toast.LENGTH_SHORT).show()

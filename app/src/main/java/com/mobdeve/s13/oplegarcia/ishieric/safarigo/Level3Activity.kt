...
class Level3Activity : AppCompatActivity() {
    ...
    private var targetItem = "Dog" // Initial target for this level
    private var points = 0

    // List of animals for each level, cycling through them
    private val animals = listOf("Dog", "Hedgehog", "Chameleon")
    private var currentIndex = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_g1level3) // Ensure this is the correct layout
    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Dog" -> "Hint: I am loyal, bark to communicate, and love to fetch. D _ _"
                "Hedgehog" -> "Hint: I have spiky quills and roll up to protect myself. H _ D G _ H _ G"
                "Chameleon" -> "Hint: I can change my color to blend into my surroundings. C H _ M _ L _ _ N"
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            currentIndex = 0
            dbHelper.setLevelCompleted("Level 3")
            Toast.makeText(this, "Level Completed! Proceed to Level 4 Yipee!!!", Toast.LENGTH_SHORT).show()
        ...
        targetImageView.setImageResource(when (targetItem) {
            ...
            "Hedgehog" -> R.drawable.hedgehog_image
            "Chameleon" -> R.drawable.chameleon_image

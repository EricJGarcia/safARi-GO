...
class Level8Activity : AppCompatActivity() {
    ...
    private var targetItem = "Koala" // Initial target for this level
    private var points = 0

    // List of animals for each level, cycling through them
    private val animals = listOf("Koala", "Chicken", "Panda")
    private var currentIndex = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_g1level8) // Ensure this is the correct layout
    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Koala" -> "Hint: I live in trees and eat eucalyptus leaves all day. K _ _ L _"
                "Chicken" -> "Hint: I lay eggs and am often kept on farms. C _ _ _ K _ N"
                "Panda" -> "Hint: I am black and white, and I love to eat bamboo. P _ _ D _"
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            currentIndex = 0
            dbHelper.setLevelCompleted("Level 8")
            Toast.makeText(this, "Level Completed! Proceed to Level 9 Yipee!!!", Toast.LENGTH_SHORT).show()

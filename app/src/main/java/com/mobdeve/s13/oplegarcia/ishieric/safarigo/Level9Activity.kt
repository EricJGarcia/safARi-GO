...
class Level9Activity : AppCompatActivity() {
    ...
    private var targetItem = "Bat" // Initial target for this level
    private var points = 0

    // List of animals for each level, cycling through them
    private val animals = listOf("Bat", "Duck", "Penguin")
    private var currentIndex = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_g1level9) // Ensure this is the correct layout
    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Bat" -> "Hint: I am the only mammal that can truly fly. B _ _"
                "Duck" -> "Hint: I quack and love swimming in ponds. D _ C _"
                "Penguin" -> "Hint: I waddle and am known for living in icy places. P _ _ G _ _ N"
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            currentIndex = 0
            dbHelper.setLevelCompleted("Level 9")
            Toast.makeText(this, "Level Completed! Proceed to Level 10 Yipee!!!", Toast.LENGTH_SHORT).show()
        ...
        targetImageView.setImageResource(when (targetItem) {
            "Bat" -> R.drawable.bat_image
            "Duck" -> R.drawable.duck_image
            "Penguin" -> R.drawable.penguin_image

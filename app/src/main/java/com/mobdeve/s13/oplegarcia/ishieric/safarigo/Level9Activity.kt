    ...
    private fun processGuess(guess: String) {
        if (guess.equals(targetItem, ignoreCase = true)) {
            ...
            val hint = when (targetItem) {
                "Bat" -> "Hint: I am the only mammal that can truly fly."
                "Duck" -> "Hint: I quack and love swimming in ponds."
                "Penguin" -> "Hint: I waddle and am known for living in icy places."
                else -> "Hint: Try again!"
            }
    ...
    private fun setNewTarget() {
        if (currentIndex >= animals.size) {
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                val intent = Intent(this, Level9Activity::class.java)
